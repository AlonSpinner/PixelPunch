...
pub struct FighterMovementMap {
    pub pressed_keyset_map : HashMap<KeyTargetSet,Arc<FighterMovementNode>>,
    pub just_pressed_keyset_map : HashMap<KeyTargetSet,Arc<FighterMovementNode>>,
    pub by_name_map : HashMap<String, Arc<FighterMovementNode>>,
}

impl FighterMovementMap {
    fn new() -> Self {
        Self{
            pressed_keyset_map : HashMap::new(),
            just_pressed_keyset_map : HashMap::new(),
            by_name_map : HashMap::new(),
        }
    }

    pub fn insert_pressed(&mut self, keyset : KeyTargetSet, movement_node : FighterMovementNode) {
        let arc_movement_node = Arc::new(movement_node);
        self.pressed_keyset_map.insert(keyset, arc_movement_node);
        self.by_name_map.insert(arc_movement_node.name.clone(), arc_movement_node);
    }

    pub fn insert_just_pressed(&mut self, keyset : KeyTargetSet, movement_node : FighterMovementNode) {
        let arc_movement_node = Arc::new(movement_node);
        self.just_pressed_keyset_map.insert(keyset, arc_movement_node);
        self.by_name_map.insert(arc_movement_node.name.clone(), arc_movement_node);
    }

    pub fn movement_names(&self) -> Vec<String> {
        self.by_name_map.values()
...
impl Default for FighterMovementMap {
    fn default() -> Self {
        ...
        map.insert_pressed(KeyTargetSet::empty(),
        FighterMovementNode{movement: FighterMovement::Idle,..default()});
        map.insert_pressed(KeyTargetSet::from([KeyTarget::Down]), 
        FighterMovementNode{movement: FighterMovement::Docking,..default()});
        map.insert_pressed(KeyTargetSet::from([KeyTarget::Left]), 
        FighterMovementNode{movement: FighterMovement::Walking{velocity: -WALKING_SPEED},
                ...
                ..default()});
        map.insert_pressed(KeyTargetSet::from([KeyTarget::Right]), 
        FighterMovementNode{movement: FighterMovement::Walking{velocity: WALKING_SPEED},
                player_enter_condition: |floor_y, position_y, previous_movement| position_y == floor_y && previous_movement.name() != "Running",
                ..default()});       
        
        map.insert_just_pressed(KeyTargetSet::from([KeyTarget::Up]),
            FighterMovementNode{movement: FighterMovement::Jumping{inital_velocity: JUMPING_SPEED, gravity: GRAVITY}
                                ,..default()});
        map.insert_just_pressed(KeyTargetSet::from([KeyTarget::LeftJustPressed]), 
        FighterMovementNode{movement: FighterMovement::Running{velocity: -RUNNING_SPEED},
                ..default()});
        map.insert_just_pressed(KeyTargetSet::from([KeyTarget::RightJustPressed]), 
        FighterMovementNode{movement: FighterMovement::Running{velocity: RUNNING_SPEED},
                player_enter_condition: |floor_y, position_y, previous_movement| position_y == floor_y && previous_movement.name() == "Walking",
                ..default()});
        map.insert_just_pressed(KeyTargetSet::from([KeyTarget::AttackJustPressed]),
